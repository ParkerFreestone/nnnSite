<template>
    <div class="landing">
        <div class="container">
            <h2> JOIN THE STRUGGLE. </h2>
            <p class="text-lg">
                The month long struggle takes place <span> Nov 1st - Nov 30th</span>, join the struggle by contacting a member of the council.
            </p>
        </div>
        
        <svg class="spacer" id="visual" viewBox="0 0 1920 300" width="1920" height="300" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"><rect x="0" y="0" width="1920" height="300" fill="#1F2937"></rect><path d="M0 136L45.7 138.3C91.3 140.7 182.7 145.3 274.2 153.7C365.7 162 457.3 174 548.8 171.7C640.3 169.3 731.7 152.7 823 142.8C914.3 133 1005.7 130 1097 137.5C1188.3 145 1279.7 163 1371.2 163.2C1462.7 163.3 1554.3 145.7 1645.8 145.5C1737.3 145.3 1828.7 162.7 1874.3 171.3L1920 180L1920 301L1874.3 301C1828.7 301 1737.3 301 1645.8 301C1554.3 301 1462.7 301 1371.2 301C1279.7 301 1188.3 301 1097 301C1005.7 301 914.3 301 823 301C731.7 301 640.3 301 548.8 301C457.3 301 365.7 301 274.2 301C182.7 301 91.3 301 45.7 301L0 301Z" fill="#a78bfa"></path><path d="M0 181L45.7 176.8C91.3 172.7 182.7 164.3 274.2 161.3C365.7 158.3 457.3 160.7 548.8 169.8C640.3 179 731.7 195 823 201.8C914.3 208.7 1005.7 206.3 1097 197.2C1188.3 188 1279.7 172 1371.2 173.5C1462.7 175 1554.3 194 1645.8 197C1737.3 200 1828.7 187 1874.3 180.5L1920 174L1920 301L1874.3 301C1828.7 301 1737.3 301 1645.8 301C1554.3 301 1462.7 301 1371.2 301C1279.7 301 1188.3 301 1097 301C1005.7 301 914.3 301 823 301C731.7 301 640.3 301 548.8 301C457.3 301 365.7 301 274.2 301C182.7 301 91.3 301 45.7 301L0 301Z" fill="#9c78f8"></path><path d="M0 211L45.7 207.8C91.3 204.7 182.7 198.3 274.2 202.5C365.7 206.7 457.3 221.3 548.8 223.8C640.3 226.3 731.7 216.7 823 210.2C914.3 203.7 1005.7 200.3 1097 204.7C1188.3 209 1279.7 221 1371.2 225.3C1462.7 229.7 1554.3 226.3 1645.8 220.7C1737.3 215 1828.7 207 1874.3 203L1920 199L1920 301L1874.3 301C1828.7 301 1737.3 301 1645.8 301C1554.3 301 1462.7 301 1371.2 301C1279.7 301 1188.3 301 1097 301C1005.7 301 914.3 301 823 301C731.7 301 640.3 301 548.8 301C457.3 301 365.7 301 274.2 301C182.7 301 91.3 301 45.7 301L0 301Z" fill="#9165f5"></path><path d="M0 249L45.7 250.2C91.3 251.3 182.7 253.7 274.2 254C365.7 254.3 457.3 252.7 548.8 253C640.3 253.3 731.7 255.7 823 250.5C914.3 245.3 1005.7 232.7 1097 231.8C1188.3 231 1279.7 242 1371.2 243.3C1462.7 244.7 1554.3 236.3 1645.8 233.2C1737.3 230 1828.7 232 1874.3 233L1920 234L1920 301L1874.3 301C1828.7 301 1737.3 301 1645.8 301C1554.3 301 1462.7 301 1371.2 301C1279.7 301 1188.3 301 1097 301C1005.7 301 914.3 301 823 301C731.7 301 640.3 301 548.8 301C457.3 301 365.7 301 274.2 301C182.7 301 91.3 301 45.7 301L0 301Z" fill="#8651f1"></path><path d="M0 258L45.7 260C91.3 262 182.7 266 274.2 265.3C365.7 264.7 457.3 259.3 548.8 259.8C640.3 260.3 731.7 266.7 823 266.2C914.3 265.7 1005.7 258.3 1097 254.5C1188.3 250.7 1279.7 250.3 1371.2 251C1462.7 251.7 1554.3 253.3 1645.8 255.3C1737.3 257.3 1828.7 259.7 1874.3 260.8L1920 262L1920 301L1874.3 301C1828.7 301 1737.3 301 1645.8 301C1554.3 301 1462.7 301 1371.2 301C1279.7 301 1188.3 301 1097 301C1005.7 301 914.3 301 823 301C731.7 301 640.3 301 548.8 301C457.3 301 365.7 301 274.2 301C182.7 301 91.3 301 45.7 301L0 301Z" fill="#7c3aed"></path></svg>
        
        <div class="rules bg-purple">
            <div class="table bg-dark">
                <div class="header">
                    General Rules For The Cause:
                </div>
                <div class="list">
                    <div class="item">
                        A $5 entry fee is required to join
                    </div>
                    <div class="item">
                        This is a full transparency clause, if you lie, you must put in an additional $5 or be disqualified.
                    </div>
                    <div class="item">
                        One free pass will be permitted on terms of sexual intercourse with MPS (Member of the preferred sex).
                    </div>
                    <div class="item">
                        This is a safe zone, meaning there is no judgement for failure of the struggle.
                    </div>
                    <div class="item">
                        Have fun, but not too much fun. ;)
                    </div>
                </div>
            </div>
        </div>

        <svg id="visual" viewBox="0 0 1920 300" width="1920" height="300" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"><rect x="0" y="0" width="1920" height="300" fill="#1F2937"></rect><path d="M0 169L45.7 178.7C91.3 188.3 182.7 207.7 274.2 212.7C365.7 217.7 457.3 208.3 548.8 204C640.3 199.7 731.7 200.3 823 208.2C914.3 216 1005.7 231 1097 231.5C1188.3 232 1279.7 218 1371.2 206.7C1462.7 195.3 1554.3 186.7 1645.8 191.7C1737.3 196.7 1828.7 215.3 1874.3 224.7L1920 234L1920 0L1874.3 0C1828.7 0 1737.3 0 1645.8 0C1554.3 0 1462.7 0 1371.2 0C1279.7 0 1188.3 0 1097 0C1005.7 0 914.3 0 823 0C731.7 0 640.3 0 548.8 0C457.3 0 365.7 0 274.2 0C182.7 0 91.3 0 45.7 0L0 0Z" fill="#a78bfa"></path><path d="M0 118L45.7 122.5C91.3 127 182.7 136 274.2 142C365.7 148 457.3 151 548.8 160.3C640.3 169.7 731.7 185.3 823 189.7C914.3 194 1005.7 187 1097 180.7C1188.3 174.3 1279.7 168.7 1371.2 158.5C1462.7 148.3 1554.3 133.7 1645.8 134.3C1737.3 135 1828.7 151 1874.3 159L1920 167L1920 0L1874.3 0C1828.7 0 1737.3 0 1645.8 0C1554.3 0 1462.7 0 1371.2 0C1279.7 0 1188.3 0 1097 0C1005.7 0 914.3 0 823 0C731.7 0 640.3 0 548.8 0C457.3 0 365.7 0 274.2 0C182.7 0 91.3 0 45.7 0L0 0Z" fill="#9c78f8"></path><path d="M0 154L45.7 144.7C91.3 135.3 182.7 116.7 274.2 115.2C365.7 113.7 457.3 129.3 548.8 128.7C640.3 128 731.7 111 823 108.2C914.3 105.3 1005.7 116.7 1097 122.2C1188.3 127.7 1279.7 127.3 1371.2 129C1462.7 130.7 1554.3 134.3 1645.8 137.2C1737.3 140 1828.7 142 1874.3 143L1920 144L1920 0L1874.3 0C1828.7 0 1737.3 0 1645.8 0C1554.3 0 1462.7 0 1371.2 0C1279.7 0 1188.3 0 1097 0C1005.7 0 914.3 0 823 0C731.7 0 640.3 0 548.8 0C457.3 0 365.7 0 274.2 0C182.7 0 91.3 0 45.7 0L0 0Z" fill="#9165f5"></path><path d="M0 102L45.7 104.3C91.3 106.7 182.7 111.3 274.2 107.7C365.7 104 457.3 92 548.8 91.3C640.3 90.7 731.7 101.3 823 99C914.3 96.7 1005.7 81.3 1097 75.7C1188.3 70 1279.7 74 1371.2 73.8C1462.7 73.7 1554.3 69.3 1645.8 68.5C1737.3 67.7 1828.7 70.3 1874.3 71.7L1920 73L1920 0L1874.3 0C1828.7 0 1737.3 0 1645.8 0C1554.3 0 1462.7 0 1371.2 0C1279.7 0 1188.3 0 1097 0C1005.7 0 914.3 0 823 0C731.7 0 640.3 0 548.8 0C457.3 0 365.7 0 274.2 0C182.7 0 91.3 0 45.7 0L0 0Z" fill="#8651f1"></path><path d="M0 33L45.7 38.3C91.3 43.7 182.7 54.3 274.2 54.5C365.7 54.7 457.3 44.3 548.8 39.8C640.3 35.3 731.7 36.7 823 43C914.3 49.3 1005.7 60.7 1097 61.2C1188.3 61.7 1279.7 51.3 1371.2 49.2C1462.7 47 1554.3 53 1645.8 56.7C1737.3 60.3 1828.7 61.7 1874.3 62.3L1920 63L1920 0L1874.3 0C1828.7 0 1737.3 0 1645.8 0C1554.3 0 1462.7 0 1371.2 0C1279.7 0 1188.3 0 1097 0C1005.7 0 914.3 0 823 0C731.7 0 640.3 0 548.8 0C457.3 0 365.7 0 274.2 0C182.7 0 91.3 0 45.7 0L0 0Z" fill="#7c3aed"></path></svg>
        <div class="container">
            <h2> The Council </h2>
            <div class="items"> 
                <div class="item2 card bg-dark">
                    <div class="name text-lg">
                        Parker
                    </div>
                    <div class="email">
                        <a href="mailto:parkerfreestone@gmail.com">parkerfreestone@gmail.com </a>
                    </div>
                </div>
                <div class="item2 card bg-dark">
                    <div class="name text-lg">
                        Nick
                    </div>
                    <div class="email">
                        <a href="mailto:motermaniac13@gmail.com">motermaniac13@gmail.com </a>
                    </div>
                </div>
                <div class="item2 card bg-dark">
                    <div class="name text-lg">
                        Bridge
                    </div>
                    <div class="email">
                        <a href="mailto:bobs1alis1@gmail.com">bobs1alis1@gmail.com </a>
                    </div>
                </div>
                <div class="item2 card bg-dark">
                    <div class="name text-lg">
                        Ben
                    </div>
                    <div class="email">
                        <a href="mailto:hamundben@gmail.com">hamundben@gmail.com </a>
                    </div>
                </div>
                <div class="item2 card bg-dark">
                    <div class="name text-lg">
                        Carter
                    </div>
                    <div class="email">
                        <a href="mailto:cklarson319@gmail.com">cklarson319@gmail.com </a>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
</template>

<script>

export default ({
    name: "LandingPage",
    
})
</script>

<style scoped>

    .rules {
        padding: 100px;
    }

    .hero-banner {
        height: 40vh;
        display: flex;
        align-items: flex-start;
        justify-content: center;
        flex-direction: center;
        text-align: center;
    }

    ul {
        list-style-type: none;
    }

    img {
        width: 20%;
    }
    
    span {
        color: #7C3AED;
    }

    .spacer {
        margin-bottom: -5px;
    }

    .table {
        width: 50vw;
        display: flex;
        align-items: center;
        justify-content: center;
        flex-direction: column;
        margin: 0 auto;
        border-radius: 20px;
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);
        overflow: hidden;
    }

    .table .header {
        font-size: 2rem;
        padding: 40px;
        background: #1F2937;
        width: 100%;
        text-align: center;
        font-weight: bold;
    }

    .table .item {
        padding: 30px;
        font-size: 1.2rem;
    }

    .container .items {
        width: 100%;
        padding: 100px;
        display: flex;
        align-items: center;
        justify-content: center;
    }

    .container .item2 {
        width: 50%;
        display: flex;
        align-items: center;
        justify-content: flex-start;
        flex-direction: column;
    }

    .container .item2 .name {
        margin-bottom: 30px;
        font-size: 2rem;
    }

    .container .item2 .email {
        font-size: 1.2rem;
    }

    .card {
        padding: 20px;
        border-radius: 20px;
        display: flex;
        align-items: center;
        justify-content: center;
        flex-direction: column;
        margin: 10px;
    }

</style>
